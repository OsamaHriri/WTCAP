...
from VerbalPOS import *
from NominalPOS import *
    ...
    def TestMatch(self, word, actions, charIndexes=None):
        ...
        if charIndexes == [None]:
            ...
            if self.Type == 'str':
                ...
                exec('from POS import *')

                isSatisfiedForOne = False
                conditionFaildForlist = []
                for i in actions.OnInnerIndexes:
                    try:
                        if (eval(self.Value) & eval('word.SurfaceFormMorphemes[i].Cliticless.POS.' + self.AttributeName) != 0):
